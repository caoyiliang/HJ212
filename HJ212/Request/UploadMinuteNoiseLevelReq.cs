...
namespace HJ212.Request
{
    internal class UploadMinuteNoiseLevelReq(string? mn, string pw, ST st, DateTime dataTime, List<NoiseLevelData> data, Version version, Func<string, string> func) : IAsyncRequest
    {
        ...
        public byte[] ToBytes()
        {
            var rs = $"QN={_QN};ST={(int)st};CN={(int)CN_Client.上传污染物分钟数据};PW={pw};MN={mn};Flag={1 | (int)version};CP=&&DataTime={dataTime:yyyyMMddHHmmss};{string.Join(";", data.Select(c => $"{c.Name}-Data={c.Data}"))}&&";
            rs = func.Invoke(rs);

...
namespace HJ212.Response
{
    internal class CalibrateRsp : IAsyncResponse<(string PolId, RspInfo RspInfo)>
    {
        ...
        public async Task AnalyticalData(byte[] bytes)
        {
            ...
            _polId = datalist.SingleOrDefault(item => item.Contains("PolId"))?.Split('=')[1] ?? throw new ArgumentException($"{GB._name} HJ212 Calibrate Error");
        ...
        public (bool Type, byte[]? CheckBytes) Check(byte[] bytes)
        {
            ...
            var rs = dstr.Split(';');
            return (rs.Where(item => item.Contains("CN=3011")).Any(), default);
        }

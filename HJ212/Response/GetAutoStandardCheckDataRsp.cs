...
{
    internal class GetAutoStandardCheckDataRsp : IAsyncResponse<(DateTime BeginTime, DateTime EndTime, RspInfo RspInfo)>
    {
        ...
        public async Task AnalyticalData(byte[] bytes)
        {
            ...
            {
                throw new ArgumentException($"HJ212 Get AutoStandardCheckData BeginTime Error");
            }
            ...
            {
                throw new ArgumentException($"HJ212 Get AutoStandardCheckData EndTime Error");
            }
        ...
        public (bool Type, byte[]? CheckBytes) Check(byte[] bytes)
        {
            ...
            return (rs.Where(item => item.Contains($"CN={(int)CN_Server.取自动标样核查校准数据}")).Any() && rs.Where(item => item.Contains("BeginTime")).Any(), default);
        }

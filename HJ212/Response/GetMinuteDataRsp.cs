...
namespace HJ212.Response
{
    internal class GetMinuteDataRsp : IAsyncResponse<(DateTime BeginTime, DateTime EndTime, RspInfo RspInfo)>
    {
        ...
        public async Task AnalyticalData(byte[] bytes)
        {
            ...
            {
                throw new ArgumentException($"HJ212 Get MinuteData BeginTime Error");
            }
            ...
            {
                throw new ArgumentException($"HJ212 Get MinuteData EndTime Error");
            }
        ...
        public (bool Type, byte[]? CheckBytes) Check(byte[] bytes)
        {
            ...
            return (rs.Where(item => item.Contains($"CN={(int)CN_Server.取污染物分钟数据}")).Any(), default);
        }

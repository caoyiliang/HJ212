...
{
    internal class GetRawDataRsp : IAsyncResponse<(DateTime BeginTime, DateTime EndTime, RspInfo RspInfo)>
    {
        ...
        public async Task AnalyticalData(byte[] bytes)
        {
            ...
            {
                throw new ArgumentException($"HJ212 Get RawData BeginTime Error");
            }
            ...
            {
                throw new ArgumentException($"HJ212 Get RawData EndTime Error");
            }
        ...
        public (bool Type, byte[]? CheckBytes) Check(byte[] bytes)
        {
            ...
            return (rs.Where(item => item.Contains($"CN={(int)CN_Server.取原始监测数据}")).Any(), default);
        }

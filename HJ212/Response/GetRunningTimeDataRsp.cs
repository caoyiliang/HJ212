...
namespace HJ212.Response
{
    internal class GetRunningTimeDataRsp : IAsyncResponse<(DateTime BeginTime, DateTime EndTime, RspInfo RspInfo)>
    {
        ...
        public async Task AnalyticalData(byte[] bytes)
        {
            ...
            {
                throw new ArgumentException($"HJ212 Get RunningTimeData BeginTime Error");
            }
            ...
            {
                throw new ArgumentException($"HJ212 Get RunningTimeData EndTime Error");
            }
        ...
        public (bool Type, byte[]? CheckBytes) Check(byte[] bytes)
        {
            ...
            return (rs.Where(item => item.Contains($"CN={(int)CN_Server.取设备运行时间日历史数据}")).Any(), default);
        }

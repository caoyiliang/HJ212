...
{
    internal class GetSNRsp : IAsyncResponse<(string PolId, RspInfo RspInfo)>
    {
        ...
        public async Task AnalyticalData(byte[] bytes)
        {
            ...
            _polId = datalist.SingleOrDefault(item => item.Contains("PolId"))?.Split('=')[1] ?? throw new ArgumentException($"{GB._name} HJ212 Get SampleExtractionTime Error");
        ...
        public (bool Type, byte[]? CheckBytes) Check(byte[] bytes)
        {
            ...
            return (rs.Where(item => item.Contains($"CN={(int)CN_Server.提取设备唯一标识}")).Any(), default);
        }

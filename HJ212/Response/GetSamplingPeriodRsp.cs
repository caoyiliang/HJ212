...
{
    internal class GetSamplingPeriodRsp : IAsyncResponse<(string PolId, RspInfo RspInfo)>
    {
        ...
        public async Task AnalyticalData(byte[] bytes)
        {
            ...
            _polId = datalist.SingleOrDefault(item => item.Contains("PolId"))?.Split('=')[1] ?? throw new ArgumentException($"{GB._name} HJ212 Get SamplingPeriod Error");
        ...
        public (bool Type, byte[]? CheckBytes) Check(byte[] bytes)
        {
            ...
            var rs = dstr.Split(';');
            return (rs.Where(item => item.Contains($"CN={(int)CN_Server.提取采样时间周期}")).Any(), default);
        }

...
{
    internal class StartAutoStandardCheckRsp : IAsyncResponse<(string PolId, RspInfo RspInfo)>
    {
        ...
        public async Task AnalyticalData(byte[] bytes)
        {
            ...
            _polId = datalist.SingleOrDefault(item => item.Contains("PolId"))?.Split('=')[1] ?? throw new ArgumentException($"HJ212 StartAutoStandardCheck PolId Error");
        ...
        public (bool Type, byte[]? CheckBytes) Check(byte[] bytes)
        {
            ...
            return (rs.Where(item => item.Contains($"CN={(int)CN_Server.启动标样核查}")).Any(), default);
        }

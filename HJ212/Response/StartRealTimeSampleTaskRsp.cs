...
{
    internal class StartRealTimeSampleTaskRsp : IAsyncResponse<(string PolId, RspInfo RspInfo)>
    {
        ...
        public async Task AnalyticalData(byte[] bytes)
        {
            ...
            _polId = datalist.SingleOrDefault(item => item.Contains("PolId"))?.Split('=')[1] ?? throw new ArgumentException($"HJ212 StartRealTimeSampleTask PolId Error");
        ...
        public (bool Type, byte[]? CheckBytes) Check(byte[] bytes)
        {
            ...
            return (rs.Where(item => item.Contains($"CN={(int)CN_Server.下发即时留样任务}")).Any(), default);
        }
